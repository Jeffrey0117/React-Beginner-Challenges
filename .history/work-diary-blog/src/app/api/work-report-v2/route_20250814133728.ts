...
export async function GET(request: Request) {
  try {
    ...
    // 計算正確的 iDisplayStart，API 是按時間倒序排列（最新在前）
    ...
    const filteredData = data.aaData.filter((item: any[]) => {
      // 主要檢查第4欄（index 3）的主題欄位
      const subject = String(item[3] || '').trim()
      
      // 如果主題明確是派工單，則過濾掉
      if (subject === '派工單' || subject.toLowerCase() === 'work order') {
        console.log(`🚫 過濾派工單: ${subject}`)
        return false
      }
      ...
      for (let i = 0; i < item.length; i++) {
        ...
        // 只有完全匹配派工單才過濾
        if (cellContent === '派工單' || 
            cellContent === 'work order' ||
            cellContent.includes('派工單處理') ||
            cellContent.includes('work order processing')) {
          console.log(`🚫 過濾派工單項目: ${cellContent.substring(0, 30)}`)
          return false
        }
      }
      
      return true // 保留所有其他項目
    })
