...
interface SalaryData {
  ...
  actualWorkingHours?: number // 實際工時
}
...
export async function GET() {
  try {
    ...
    // 計算工時統計 - 使用實際工時
    const workingHours = salaryRecords.map(record => 
      record.actualWorkingHours || Math.round(record.salary / record.hourlyWage)
    )
    
    const totalWorkingHours = workingHours.reduce((sum, hours) => sum + hours, 0)
    const averageWorkingHours = workingHours.length > 0 ? Math.round(totalWorkingHours / workingHours.length) : 0
