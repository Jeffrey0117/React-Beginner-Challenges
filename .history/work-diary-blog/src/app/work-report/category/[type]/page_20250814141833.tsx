...
import { useState, useEffect } from '        const allTypes = [...new Set(allReports.map((item: any[]) => item[3] || ''))].filter((type: string) => type.trim() !== '')
        console.log('📋 所有可用的日報類型:', allTypes)
        console.log(`🔍 正在尋找的類型: "${categoryType}"`)
        
        // 檢查是否有相似的類型
        const similarTypes = allTypes.filter(type => type.includes(categoryType) || categoryType.includes(type))
...
export default function CategoryPage() {
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      if (!cacheResponse.ok) {
        throw new Error(`快取 API 錯誤: ${cacheResponse.status}`)
      }
